...
function Sample() {
  ...
  return <div className={`class1 class2 ${activate && "class3"}`}>contents...</div>;
}
```
...
function Sample() {
  const activate = useState(true);
  return <div className={clsx(["class1", "class2", { "class3": activate }])}>contents...</div>;
}
```
...
Variants is an example of declarative dynamic styling.
...
function Block() {
  return (
    <div className={ blockName.block({ /* with Modifier */ }) }>
      <div className={ blockName.element({ /* with Modifier */ }) }></div>
    </div>
  );
}
```
...
3. Should be available for use with `cssVariant`.
...
**Static Usage:**
```typescript
const myCSS = css([myRule({ color: "red", background: "blue", size: "5px" }), { borderRadis: 999 }]);
```
  ...
  return (
    <div>
      {[...Array(10)].map((_, index) => (
        <div key={index} className={css`${generateBubbleVars()} width: var(--x); height: var(--y);`} />
      ))}
    </div>
  )
}
...
function App() {
  return (
    <div>
      {[...Array(10)].map((_, index) => (
        <div key={index} style={myRule.props({ x: randomBetween(10, 90), y: randomBetween(15, 85) })} />
      ))}
    </div>
  )
}
...
PandaCSS is implementing a very powerful variants feature([Recipes](https://panda-css.com/docs/concepts/recipes), [Slot Recipes](https://panda-css.com/docs/concepts/slot-recipes)).
